"""Multi-stage stochastic programming policy for Task 3."""
...
def _build_tree(state, L):
    ...
    while q:
        ...
        for j in range(b):
            ...
            if len(members) == 0:
                prob_frac = 0.0
                p_val = _price_sample(par["price"], par["price_prev"])
                o1_val, o2_val = _occ_sample(par["occ1"], par["occ2"])
            else:
